...
int main(void) {
	...
	while (1) {
		...
		HAL_ADC_Stop(&hadc1);
		double uvValue = getUVValue(adcValue);
		char uvString[16] = "";
		sprintf(uvString,"%f6", uvValue);
		PrintString(ssd1360_d, "UV value", 2, 0);
		PrintString(ssd1360_d, uvString, 3, 3);
		PrintString(ssd1360_d, "milli watt per", 5, 0);
		PrintString(ssd1360_d, "square cm", 6, 0);
		OLED_Write_Display(ssd1360_d);
		HAL_Delay(2000);
	}
}
...
static const double ZERO_VAL = 1132;
static const double TWELVE_VAL = 2978;
static const double DIFF_UV1 = 144.75;

static double getUVValue(uint32_t adcValue) {
	double dblAdcVal = (double)adcValue;
	return (dblAdcVal - ZERO_VAL) / DIFF_UV1;
}
